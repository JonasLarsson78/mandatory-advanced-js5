...
if (error.response.status === 400) {
    ...
    updateErrorMessage("The server couldn't handle the input. Please try again.")
} 
...
else if (error.response.status < 600 && error.response.status > 499){
    //An error occurred on the Dropbox servers. Check status.dropbox.com for announcements about Dropbox service issues.
    updateErrorMessage("Something went wrong with the server. Please try again in a couple of minutes.")
}
else {
    updateErrorMessage("Something went wrong. Please try again in a couple of minutes.")
}
}
